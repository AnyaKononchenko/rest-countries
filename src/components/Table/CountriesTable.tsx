...
const CountriesTable = (props: TableProps) => {
  ...
  const handleSort = (event: React.MouseEvent<HTMLTableCellElement, MouseEvent>) => {
    ...
    setTableState({
      ...
      sortDirection: tableState.sortDirection ? invertDirection[tableState.sortDirection] : 'desc',
    });
  }
  ...
  const sortedHeaders = headersToSort.map((header, index) =>
    <StyledTableHeader key={index} width='15%' onClick={handleSort} sx={{ '&:hover': { 'cursor': 'pointer' } }}>
      {tableState.sortColumn === header.toLowerCase() &&
        <>
          {tableState.sortDirection === 'desc' ? <HiOutlineArrowSmDown /> : <HiOutlineArrowSmUp />}
        </>
      }
  ...
  const countriesRows = (array: Country[]) => array.map((country: Country, index: number) => (
    <StyledTableRow key={index} >
      ...
      <TableCell><Link to='/country' state={country.name.common}><Typography variant='body2' color='primary'>{country.name.common}</Typography></Link></TableCell>
    ...
    <>
      <TableContainer component={Paper}>
        <Table aria-label='list of countries'>
          ...
          <TableBody>
            {countriesRows(orderBy(countries, tableState.sortColumn, tableState.sortDirection))
